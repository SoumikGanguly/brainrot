...
type OemBrand = 'Samsung' | 'Xiaomi' | 'Realme' | 'Oppo' | 'OnePlus' | 'Vivo' | 'Motorola' | 'Pixel' | 'Other';
...
const SCORE_BANDS = [
  { range: '90 - 100', label: 'Focused', copy: 'You’re in control. Keep going!', bg: '#E8F9EE', text: '#16A34A' },
  { range: '70 - 89', label: 'Healthy', copy: 'Good balance. Room to improve.', bg: '#FFF4DD', text: '#F59E0B' },
  { range: '50 - 69', label: 'Foggy', copy: 'Distractions are adding up.', bg: '#FFEADF', text: '#F97316' },
  { range: '0 - 49', label: 'Exhausted', copy: 'Your brain needs a reset.', bg: '#FCE7F3', text: '#EF4444' },
] as const;
...
const OEM_GUIDES: Record<OemBrand, { subtitle: string; steps: string[] }> = {
  Samsung: {
    ...
    steps: ['Open Settings', 'Tap Accessibility', 'Open Installed apps', 'Select Brainrot', 'Turn the service on'],
  },
  Xiaomi: {
    subtitle: 'How to enable Accessibility',
    steps: ['Open Settings', 'Tap Additional settings', 'Open Accessibility', 'Select Downloaded apps', 'Turn Brainrot on'],
  },
  Realme: {
    subtitle: 'How to enable Accessibility',
    steps: ['Open Settings', 'Tap Accessibility', 'Open Downloaded Apps', 'Find Brainrot', 'Enable the service'],
  },
  Oppo: {
    subtitle: 'How to enable Accessibility',
    steps: ['Open Settings', 'Tap Additional settings', 'Open Accessibility', 'Tap Downloaded apps', 'Allow Brainrot'],
  },
  OnePlus: {
    subtitle: 'How to enable Accessibility',
    steps: ['Open Settings', 'Tap Additional settings', 'Open Accessibility', 'Tap Downloaded apps', 'Enable Brainrot'],
  },
  Vivo: {
    subtitle: 'How to enable Accessibility',
    steps: ['Open Settings', 'Tap Shortcuts & accessibility', 'Open Accessibility', 'Tap Downloaded apps', 'Allow Brainrot'],
  },
  Motorola: {
    subtitle: 'How to enable Accessibility',
    steps: ['Open Settings', 'Tap Accessibility', 'Open Downloaded apps', 'Select Brainrot', 'Turn it on'],
  },
  Pixel: {
    subtitle: 'How to enable Accessibility',
    steps: ['Open Settings', 'Tap Accessibility', 'Open Downloaded apps', 'Select Brainrot', 'Use the toggle to enable'],
  },
  Other: {
    subtitle: 'How to enable Accessibility',
    steps: ['Open Settings', 'Search for Accessibility', 'Look for Downloaded or Installed apps', 'Find Brainrot', 'Enable the service'],
  },
};

export default function OnboardingFlow({ preview = false }: OnboardingFlowProps) {
  const [currentStep, setCurrentStep] = useState(0);
  const [usageGranted, setUsageGranted] = useState(false);
  const [accessibilityGranted, setAccessibilityGranted] = useState(false);
  const [loadingApps, setLoadingApps] = useState(true);
  const [installedApps, setInstalledApps] = useState<InstalledAppOption[]>(COMMON_APP_OPTIONS);
  ...
  const [permissionSuccess, setPermissionSuccess] = useState<Record<'usage' | 'accessibility', boolean>>({
    usage: false,
    accessibility: false,
  });
  const [selectedGuideBrand, setSelectedGuideBrand] = useState<OemBrand | null>(null);
  ...
  const refreshPermissionState = useCallback(async () => {
    ...
    if (accessibility && !accessibilityGranted) {
      setPermissionSuccess((previous) => ({ ...previous, accessibility: true }));
      setTimeout(() => {
        setPermissionSuccess((previous) => ({ ...previous, accessibility: false }));
      }, 1600);
    }
  ...
  async function finishOnboarding() {
    ...
    try {
      ...
      await MonitoredAppsService.getInstance().replaceMonitoredApps(appsToMonitor);
      ...
      await Promise.all([
        ...
        database.setMeta('blocking_mode', selectedProtectionLevel === 'hard' ? 'hard' : 'soft'),
        ...
        database.setMeta('onboarding_selected_app', selectedAppEntry.packageName),
        database.setMeta('onboarding_selected_app_name', selectedAppEntry.appName),
        database.setMeta('onboarding_protection_level', selectedProtectionLevel),
      ...
      Alert.alert('Setup Error', 'We could not finish setup. Please try again.');
    }
  }
    ...
    <View className="mb-6 flex-row items-center justify-between">
      <View className="mr-4 flex-1">
        <View className="mb-2 flex-row items-center justify-between">
          <Text className="font-body text-secondary text-slate-600">{currentStep + 1}/{TOTAL_STEPS}</Text>
          {currentStep < TOTAL_STEPS - 1 ? (
            <TouchableOpacity onPress={() => void jumpToFinalStep()}>
              <Text className="font-heading-semibold text-secondary text-accent">Skip</Text>
            </TouchableOpacity>
          ) : (
            <View />
          )}
  ...
  const renderPermissionPill = (granted: boolean, flash: boolean, copy: string) => (
    <View className={`mt-4 flex-row items-center rounded-2xl px-4 py-3 ${granted ? 'bg-emerald-50' : 'bg-slate-100'}`}>
      <Ionicons
        name={granted ? 'checkmark-circle' : 'time-outline'}
        size={18}
        color={granted ? '#16A34A' : '#64748B'}
      />
      <Text className={`ml-2 flex-1 font-body text-secondary ${granted ? 'text-emerald-800' : 'text-slate-600'}`}>
        {granted ? copy : 'Still waiting for permission.'}
      </Text>
      {flash ? <Text className="ml-2 text-xs font-heading-semibold text-emerald-700">Done</Text> : null}
    </View>
  );
          ...
          >
            <View className="mt-4 flex-1 items-center justify-center">
              <View className="relative h-72 w-full items-center justify-center">
                <FloatingAppBubble icon="logo-instagram" color="#EC4899" style="left-4 top-10" />
                <FloatingAppBubble icon="logo-youtube" color="#FF3B30" style="right-4 top-8" />
                <FloatingAppBubble icon="musical-notes" color="#111827" style="left-0 bottom-16" />
                <FloatingAppBubble icon="logo-tiktok" color="#111827" style="right-8 bottom-20" />
                <View className="h-44 w-44 items-center justify-center rounded-full" style={{ backgroundColor: '#EFE7FF' }}>
                  <View className="h-36 w-36 items-center justify-center rounded-full" style={{ backgroundColor: '#8B5CF6' }}>
      ...
      case 1:
        return (
          <StepLayout
            ...
            onPrimary={() => void goNext()}
          >
            <View className="mt-6 flex-1 flex-row items-center">
              <View className="mr-4 flex-1 rounded-[28px] border border-slate-200 bg-white px-5 py-6">
                <View className="flex-row">
                  <View className="mr-4 justify-between">
                    {['7 AM', '9 AM', '12 PM', '3 PM', '6 PM', '9 PM', '12 AM'].map((label) => (
                      <Text key={label} className="text-xs font-body text-slate-500">
                        {label}
                      </Text>
                    ))}
                  </View>
                  <View className="mr-5 items-center">
                    <View className="h-full w-0.5 bg-violet-200" />
                    {Array.from({ length: 7 }).map((_, index) => (
                      <View
                        key={index}
                        className="absolute h-2 w-2 rounded-full bg-violet-500"
                        style={{ top: 12 + index * 34 }}
                      />
                    ))}
                  </View>
                  <View className="flex-1 justify-between py-1">
                    {[62, 34, 52, 44, 59, 41, 57, 33, 48, 54, 38, 61, 45, 50].map((width, index) => (
                      <View
                        key={index}
                        className="h-2 rounded-full bg-pink-500"
                        style={{ width: `${width}%`, marginBottom: index === 13 ? 0 : 8 }}
                      />
                    ))}
                  </View>
                </View>
              </View>
              <View className="w-28 rounded-[24px] border border-violet-200 bg-violet-50 px-4 py-5">
                <Text className="font-body text-secondary text-slate-600">Instagram opened</Text>
                <Text className="mt-3 text-4xl font-bold text-accent">40</Text>
                <Text className="mt-1 font-body text-secondary text-slate-600">times today</Text>
              </View>
            </View>
          </StepLayout>
        );
      ...
      case 2:
        return (
          <StepLayout
            ...
            onPrimary={() => void goNext()}
          >
            <Image source={ASSETS.brainHealth} resizeMode="contain" className="mt-5 h-28 w-full" />
            <View className="mt-4">
              {SCORE_BANDS.map((band) => (
                <View key={band.label} className="mb-3 flex-row items-center rounded-[24px] border border-slate-100 bg-white px-4 py-4">
                  <View className="mr-4 h-12 w-12 items-center justify-center rounded-full" style={{ backgroundColor: band.bg }}>
                    <Text className="text-2xl">🧠</Text>
                  </View>
                  <View className="flex-1">
                    <Text style={{ color: band.text }} className="font-heading-bold text-card-title">
                      {band.range}
                    </Text>
                    <Text className="mt-1 font-body text-secondary text-slate-600">{band.copy}</Text>
                  </View>
                  <Text style={{ color: band.text }} className="font-heading-semibold text-card-title">
                    {band.label}
                  </Text>
                </View>
              ))}
      ...
      case 3:
        return (
          <StepLayout
            ...
            onPrimary={() => void goNext()}
          >
            <View className="mt-6 rounded-[28px] border border-slate-200 bg-white px-4 py-5">
              <Image source={ASSETS.replay} resizeMode="contain" className="h-72 w-full" />
            </View>
          </StepLayout>
        );
        ...
        return (
          ...
          >
            <View className="mt-6 flex-1 items-center justify-center">
              <Image source={ASSETS.blocking} resizeMode="contain" className="h-80 w-full" />
            </View>
          </StepLayout>
        );
      ...
      case 5:
        return (
          <StepLayout
            ...
            onPrimary={usageGranted ? () => void goNext() : () => void openUsageAccess()}
            ...
            <View className="mt-5 rounded-[28px] bg-white px-5 py-5">
              {[
                ...
                <View key={item} className="mb-3 flex-row items-center">
                  ...
                  <Text className="ml-3 font-body text-body text-slate-700">{item}</Text>
                </View>
              ))}
              ...
              <Image source={ASSETS.permissions} resizeMode="contain" className="mt-5 h-44 w-full" />
            </View>
            {renderPermissionPill(usageGranted, permissionSuccess.usage, 'Usage access granted.')}
          </StepLayout>
        );
      ...
      case 6:
        return (
          <StepLayout
            ...
            primaryLabel={accessibilityGranted ? 'Continue' : 'Enable Protection'}
            onPrimary={accessibilityGranted ? () => void goNext() : () => void openAccessibility()}
            secondaryLabel={accessibilityGranted ? undefined : 'I’ll set this up later'}
            onSecondary={accessibilityGranted ? undefined : () => void goNext()}
            footer="We’ll guide you step by step"
          >
            <View className="mt-5 rounded-[28px] bg-white px-5 py-5">
              <View className="rounded-2xl bg-violet-50 px-4 py-4">
                <Text className="font-heading-semibold text-card-title text-accent">Without it, you won’t get:</Text>
                {['No blocking', 'No cooldowns', 'No accountability mode'].map((item) => (
              ...
              <Image source={ASSETS.blocking} resizeMode="contain" className="mt-5 h-52 w-full" />
            </View>
            {renderPermissionPill(accessibilityGranted, permissionSuccess.accessibility, 'Focus Protection enabled.')}
          </StepLayout>
        );
              ...
              {(Object.keys(OEM_GUIDES) as OemBrand[]).map((brand) => (
                ...
                >
                  <View className="mb-2 h-9 w-9 items-center justify-center rounded-full bg-slate-100">
                    <Text className="font-heading-bold text-secondary text-accent">{brand[0]}</Text>
                  </View>
                  <Text className="text-center font-body text-secondary text-slate-700">{brand}</Text>
                </TouchableOpacity>
              ))}
            </View>
            <View className="mt-4 flex-row rounded-2xl bg-violet-50 px-4 py-3">
              <Ionicons name="shield-checkmark-outline" size={18} color="#5B4CF0" />
              <Text className="ml-3 flex-1 font-body text-secondary text-slate-700">
                We only need Accessibility permission to block apps and protect your focus.
              </Text>
            </View>
          </StepLayout>
        );
        ...
        return (
          ...
          >
            <View className="mt-4 flex-1 items-center">
              <View className="h-44 w-44 items-center justify-center rounded-full bg-violet-100">
                <Text className="text-7xl">🧠</Text>
              </View>
              <Image source={ASSETS.completeCards} resizeMode="contain" className="mt-6 h-52 w-full" />
            </View>
          </StepLayout>
        );
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <View className="flex-1 px-5 py-4">
        ...
        {currentStep > 0 ? (
          <TouchableOpacity onPress={() => void goBack()} className="mb-4 self-start rounded-full bg-slate-100 px-4 py-2">
            <Text className="font-heading-semibold text-secondary text-slate-700">Back</Text>
          </TouchableOpacity>
        ) : null}
  ...
  return (
    <View className="flex-1">
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        <Text className="font-heading-bold text-4xl leading-[42px] text-slate-900">{title}</Text>
        <Text className="mt-3 font-body text-body leading-7 text-slate-600">{body}</Text>
        {children}
      </ScrollView>
      <View className="pt-4">
        {secondaryLabel && onSecondary ? (
          ...
          >
            <Text className="font-heading-semibold text-card-title text-slate-700">{secondaryLabel}</Text>
          </TouchableOpacity>
        ) : null}
        <TouchableOpacity
          onPress={onPrimary}
          className="flex-row items-center justify-center rounded-2xl bg-accent px-4 py-4"
        >
          <Text className="font-heading-semibold text-card-title text-white">{primaryLabel}</Text>
          <Ionicons name="arrow-forward" size={18} color="#FFFFFF" style={{ marginLeft: 10 }} />
        </TouchableOpacity>
        {footer ? (
          <Text className="mt-4 text-center font-body text-secondary text-slate-500">{footer}</Text>
        ) : null}
      </View>
    </View>
  );
...
function FloatingAppBubble({
  ...
  return (
    <View className={`absolute h-14 w-14 items-center justify-center rounded-2xl bg-white shadow-sm ${style}`}>
      ...
      <View className="flex-1 justify-end bg-black/35">
        ...
        <View className="rounded-t-[32px] bg-white px-5 pb-8 pt-5">
          <View className="mb-5 flex-row items-center justify-between">
            <View>
              <Text className="font-heading-bold text-section text-slate-900">{brand}</Text>
              <Text className="mt-1 font-body text-secondary text-slate-600">{guide.subtitle}</Text>
            </View>
          ...
          {guide.steps.map((step, index) => (
            <View key={step} className="mb-3 flex-row rounded-2xl bg-slate-50 px-4 py-4">
              <View className="mr-4 h-7 w-7 items-center justify-center rounded-full bg-accent">
                <Text className="font-heading-semibold text-secondary text-white">{index + 1}</Text>
              </View>
              <Text className="flex-1 font-body text-body text-slate-700">{step}</Text>
            </View>
          ))}

          <TouchableOpacity onPress={onOpenSettings} className="mt-3 flex-row items-center justify-center rounded-2xl bg-accent px-4 py-4">
          ...
          <TouchableOpacity
            onPress={() => Alert.alert('Support', 'If this setting still feels hidden, try searching for Accessibility in your phone settings.')}
            className="mt-4 items-center"
          >
            <Text className="font-heading-semibold text-secondary text-accent">Contact Support</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
