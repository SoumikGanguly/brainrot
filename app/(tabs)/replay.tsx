...
import { AppState, AppStateStatus, ScrollView, Text, TouchableOpacity, View } from 'react-native';
...
import { UnifiedUsageService, type ManufacturerPermissionInfo } from '../../services/UnifiedUsageService';
...
const replayExpressions = {
  happy: require('../../assets/expressions/happy.svg'),
  healthy: require('../../assets/expressions/healthy.svg'),
  confused: require('../../assets/expressions/confused.svg'),
  disappointed: require('../../assets/expressions/disappointed.svg'),
  exhausted: require('../../assets/expressions/exhausted.svg'),
} as const;
...
function getAppVisual(appName: string, packageName: string): { icon: keyof typeof Ionicons.glyphMap; bg: string; accent: string } {
...
export default function ReplayScreen() {
  const [selectedDay, setSelectedDay] = useState<ReplayDay>('yesterday');
  ...
  async function loadReplayData(date: string) {
    try {
      ...
      const insights = await DailyInsightsService.getInstance().getDailyInsights(date, {
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
            ...
            <View className="flex-row items-start">
              ...
              <View className="flex-1">
                ...
                <Text className="mb-3 font-body text-secondary text-yellow-700">
                  Replay needs usage access so it can rebuild your distraction timeline.
                </Text>
                <TouchableOpacity
                  onPress={async () => {
                ...
                >
                  <Text className="font-heading-semibold text-secondary text-white">Grant Permission</Text>
                </TouchableOpacity>
                ...
                {manufacturerInfo?.needsSpecialPermission && (
                  <View className="mt-3 rounded-lg border border-yellow-200 bg-yellow-100 p-3">
                    ...
                    {manufacturerInfo.canOpenDirectly && (
                      ...
                      >
                        <Text className="font-heading-semibold text-secondary text-white">Open OEM Settings</Text>
                      </TouchableOpacity>
                    )}
                  </View>
                )}
        ...
        <Card className="mx-md mb-md">
          <View className="flex-row rounded-2xl bg-slate-100 p-1">
            ...
            >
              <Text className={`text-center font-heading-semibold ${selectedDay === 'yesterday' ? 'text-text' : 'text-muted'}`}>
                Yesterday&apos;s Replay
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
            ...
            >
              <Text className={`text-center font-heading-semibold ${selectedDay === 'today' ? 'text-text' : 'text-muted'}`}>
                Today&apos;s Replay
              </Text>
            </TouchableOpacity>
          </View>
        ...
        <Card className="mx-md mb-md">
          <View className="flex-row items-center justify-between">
            <View className="flex-1 pr-sm">
              <Text className="font-heading-bold text-section text-text">You wasted</Text>
              <Text className="mt-sm text-4xl font-bold text-danger">{formatTime(wastedTimeMs)}</Text>
              <Text className="mt-2 font-body text-secondary text-muted">
                opening apps {summary?.totalMonitoredOpens ?? replayEntries.length} times
              </Text>
            </View>
            <ExpressionBadge source={replayExpressions.disappointed} size={88} />
          </View>
        </Card>

        <Card className="mx-md mb-md">
          <Text className="mb-md font-heading-bold text-section text-text">Session Replay</Text>
          {replayEntries.length === 0 ? (
            <Text className="font-body text-body text-muted">{emptyStateText}</Text>
          ) : (
            <ScrollView
              nestedScrollEnabled
            ...
            >
              {replayEntries.map((entry, index) => {
                ...
                return (
                  <View key={`${entry.packageName}-${entry.startedAt}-${index}`} className="flex-row pb-md last:pb-0">
                    <View className="w-16 pt-3">
                      <Text className="font-body-semibold text-secondary text-muted">{startTime}</Text>
                    </View>

                    <View className="mr-md items-center">
                      <View className="mt-3 h-3 w-3 rounded-full" style={{ backgroundColor: momentTheme.dot }} />
                      {index < replayEntries.length - 1 ? (
                        <View className="mt-1 w-0.5 flex-1" style={{ backgroundColor: `${momentTheme.dot}55` }} />
                      ) : (
                    ...
                    <View className="flex-1 rounded-3xl border border-slate-200 bg-card px-4 py-4">
                      <View className="flex-row items-start justify-between">
                        <View className="flex-row flex-1 items-center">
                          ...
                          <Text className="flex-1 font-heading-semibold text-card-title text-text">{entry.appName}</Text>
                        </View>
                        <Text className="ml-3 font-heading-bold text-card-title" style={{ color: visual.bg }}>
                          +{formatTime(entry.durationMs)}
                        </Text>
                      </View>

                      <View className="mt-3 flex-row justify-end">
                        <View className="rounded-full px-3 py-1" style={{ backgroundColor: momentTheme.pillBackground }}>
                          <Text className="font-body-semibold text-secondary" style={{ color: momentTheme.pillText }}>
                            {entry.moment}
                          </Text>
                        </View>
                      </View>
        ...
        <View className="mx-md mb-xl flex-row">
          <Card className="mr-sm flex-1">
            <Text className="font-heading-bold text-card-title text-text">Biggest Time Leak</Text>
            {biggestTimeLeak ? (
              <>
                <View className="mt-3 flex-row items-center justify-between">
                  <View className="mr-2 flex-1">
                    <View className="flex-row items-center">
                      ...
                      <View className="ml-3 flex-1">
                        <Text className="font-heading-semibold text-card-title text-text">{biggestTimeLeak.appName}</Text>
                        <Text className="mt-1 text-2xl font-bold text-danger">{formatTime(biggestTimeLeak.totalTimeMs)}</Text>
                  ...
                  <ExpressionBadge source={replayExpressions.disappointed} size={72} />
                </View>
              </>
            ) : (
              <View className="mt-3 flex-row items-center justify-between">
                <Text className="mr-3 flex-1 font-body text-secondary text-muted">No obvious villain today.</Text>
                <ExpressionBadge source={replayExpressions.healthy} size={68} />
              </View>
            )}
          </Card>
          ...
          <Card className="ml-sm flex-1">
            <View className="flex-row items-center justify-between">
              <View className="flex-1 pr-2">
                <Text className="font-heading-bold text-card-title text-text">Brain Score</Text>
                <Text className="mt-3 text-4xl font-bold text-accent">{getBrainScoreText(summary)}</Text>
                <Text className="mt-2 font-body text-secondary text-muted">
                  {summary?.brainHealthStatus || 'No rating yet'}
                </Text>
              </View>
              <ExpressionBadge source={getBrainExpression(summary?.focusScore ?? summary?.brainScore)} size={78} />
            </View>
          </Card>
        </View>
      </ScrollView>
...
function AppBadge({ appName, packageName, size = 44 }: { appName: string; packageName: string; size?: number }) {
  ...
  return (
    ...
    >
      {hasBrandIcon ? (
        <Ionicons name={visual.icon} size={Math.round(size * 0.5)} color="#FFFFFF" />
      ) : (
        <Text className="font-heading-bold text-white">{getAppInitials(appName)}</Text>
      )}
    </View>
  );
}
...
function ExpressionBadge({
  ...
  return (
    ...
    >
      <VectorImage source={source} style={{ width: size, height: size }} />
    </View>
  );
}
