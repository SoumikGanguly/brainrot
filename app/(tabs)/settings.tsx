...
import { Alert, type AlertButton, AppState, Image, ScrollView, Switch, Text, TouchableOpacity, View } from 'react-native';
...
type PermissionState = { usage: boolean; overlay: boolean; accessibility: boolean; notifications: boolean };
type ViewApp = { packageName: string; appName: string; isRecommended: boolean; isCurrentlyMonitored: boolean };

export default function SettingsScreen() {
  const [apps, setApps] = useState<ViewApp[]>([]);
  const [permissions, setPermissions] = useState<PermissionState>({ usage: false, overlay: false, accessibility: false, notifications: false });
  ...
  useEffect(() => {
    ...
    if (response.type === 'error') {
      setAccountActionLoading(false);
      Alert.alert('Google Sign-In Failed', response.error?.message || 'Unknown OAuth error.');
      return;
    }
  ...
  const refresh = async () => {
    setLoading(true);
    try {
      const [installedApps, monitoredPackages, usage, overlay, accessibility, notifications] = await Promise.all([
      ...
      setState({
        ...
        lastCloudSyncAt: parseInt((await database.getMeta('cloud_last_sync_at')) || '0', 10),
      });
    } finally {
      setLoading(false);
    }
  ...
  const previewBlockingScreen = async (mode: 'soft' | 'hard') => {
    try {
      if (mode === 'soft' && !permissions.overlay) {
        ...
        if (!granted) {
          Alert.alert('Overlay Required', 'Grant Display over other apps to preview the soft block screen.');
          await refresh();
          return;
        }
      }
      ...
      if (mode === 'hard' && !permissions.accessibility) {
        ...
        if (!granted) {
          Alert.alert('Accessibility Required', 'Enable Accessibility to preview the hard block screen.');
          await refresh();
          return;
        }
      }

      await UnifiedUsageService.showBlockingOverlay('dev.preview.app', mode === 'soft' ? 'Soft Block Preview' : 'Hard Block Preview', mode);
    } catch (error) {
  ...
  const signInWithGoogle = async () => {
    if (!firebaseGoogleClientIds.webClientId) {
      Alert.alert('Firebase Not Configured', 'Add your Google web client ID to the Expo env file first.');
      return;
    }

    if (!request) {
      Alert.alert('Google Sign-In Not Ready', 'The Google sign-in request is still initializing.');
      return;
    }
  ...
  const debugDuplicateIssue = async () => {
    ...
    const duplicates = rawEntries.reduce<Record<string, number>>((acc, entry) => {
  ...
  if (loading) {
    return <SafeAreaView className="flex-1 bg-bg"><View className="flex-1 items-center justify-center"><Text className="font-body text-body text-muted">Loading settings...</Text></View></SafeAreaView>;
  }
    ...
    <SafeAreaView className="flex-1 bg-bg">
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        <Card className="mx-md mb-md">
          <Text className="mb-sm font-heading-bold text-section text-text">Protect Your Data</Text>
          <Text className="mb-md font-body text-secondary text-muted">
            Sign in with Google to back up your settings, monitored apps, blocked apps, and up to 90 days of daily summaries to Firestore.
          </Text>
          {authUser ? (
            <View>
              <View className="flex-row items-center rounded-xl border border-slate-200 bg-card p-4">
                {authUser.photoURL ? (
                  <Image source={{ uri: authUser.photoURL }} className="h-12 w-12 rounded-full" />
                ) : (
                ...
                <View className="ml-3 flex-1">
                  ...
                  <Text className="font-body text-secondary text-muted">{authUser.email || authUser.uid}</Text>
            ...
            >
              ...
              <Text className="ml-3 font-heading-semibold text-card-title text-text">
                {accountActionLoading ? 'Connecting...' : 'Continue with Google'}
              </Text>
            </TouchableOpacity>
          )}
        </Card>
        ...
        <Card className="mx-md mb-md">
          <Text className="mb-sm font-heading-bold text-section text-text">Focus Tracking</Text>
          ...
          <Text className="mt-sm font-body text-secondary text-muted">Background reliability may require battery optimization exemptions on some devices.</Text>
        </Card>

        <Card className="mx-md mb-md">
          <Text className="mb-sm font-heading-bold text-section text-text">Permissions</Text>
          <PermissionRow label="Notifications" granted={permissions.notifications} onPress={() => CapabilitiesService.ensureNotificationPermission().then(() => void refresh())} />
        ...
        <Card className="mx-md mb-md">
          <Text className="mb-sm font-heading-bold text-section text-text">Privacy</Text>
          <ToggleRow label="Product Analytics" value={state.analyticsEnabled} onValueChange={(value) => TelemetryService.setEnabled(value).then(() => void refresh())} />
          <Text className="mt-sm font-body text-secondary text-muted">
            Crash reporting stays on through Sentry so app failures can still be diagnosed. This toggle controls PostHog product analytics only.
          </Text>
        ...
        {__DEV__ && (
          <Card className="mx-md mb-md bg-gray-50">
            <Text className="mb-sm font-heading-bold text-section text-text">Dev Blocking Preview</Text>
            <Text className="mb-md font-body text-secondary text-muted">
              Open the native soft and hard block screens directly for testing.
            </Text>
            <View className="flex-row">
              <TouchableOpacity onPress={() => void previewBlockingScreen('soft')} className="flex-1 rounded-lg bg-accent px-4 py-3 items-center">
                <Text className="font-heading-semibold text-secondary text-white">Preview Soft Block</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => void previewBlockingScreen('hard')} className="flex-1 rounded-lg bg-danger ml-sm px-4 py-3 items-center">
                <Text className="font-heading-semibold text-secondary text-white">Preview Hard Block</Text>
              </TouchableOpacity>
            </View>
            <TouchableOpacity onPress={() => void previewOnboardingFlow()} className="mt-sm rounded-lg bg-surface border border-gray-200 px-4 py-3 items-center">
              <Text className="font-heading-semibold text-secondary text-text">Check Onboarding Flow</Text>
            </TouchableOpacity>
          </Card>
        )}
        ...
        {__DEV__ && (
          <Card className="mx-md mb-md bg-gray-50">
            <Text className="mb-sm font-heading-bold text-section text-text">Dev Diagnostics</Text>
            <Text className="mb-md font-body text-secondary text-muted">
              Use these helpers for native module verification and duplicate-usage investigation.
            </Text>
            <TouchableOpacity onPress={() => void runNativeModuleTest()} className="rounded-lg bg-white border border-gray-200 px-4 py-3 items-center mb-sm">
              <Text className="font-heading-semibold text-secondary text-text">Run Full Native Test</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => void cleanTodayDuplicates()} className="rounded-lg bg-white border border-gray-200 px-4 py-3 items-center mb-sm">
              <Text className="font-heading-semibold text-secondary text-text">Clean Today's Duplicates</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => void debugDuplicateIssue()} className="rounded-lg bg-white border border-gray-200 px-4 py-3 items-center">
              <Text className="font-heading-semibold text-secondary text-text">Debug Duplicate Usage</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => void inspectTodaySummary()} className="rounded-lg bg-white border border-gray-200 px-4 py-3 items-center mt-sm">
              <Text className="font-heading-semibold text-secondary text-text">Inspect Today's Summary</Text>
            </TouchableOpacity>
          </Card>
        )}
      </ScrollView>
...
function LinkRow({ label }: { label: string }) {
  return <View className="flex-row items-center justify-between"><Text className="font-body text-body text-text">{label}</Text><Ionicons name="chevron-forward" size={20} color="#64748B" /></View>;
}
